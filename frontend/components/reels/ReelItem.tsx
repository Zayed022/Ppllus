...
export default function ReelItem({
  ...
  useEffect(() => {
    ...
    accumulatedWatch.current = 0;
    lastPosition.current = 0;
    lastSentReward.current = 0;
  }, [reel._id]);
  ...
  useEffect(() => {
    return () => {
      if (accumulatedWatch.current > lastSentReward.current) {
        recordReelView(
          reel._id,
          Math.floor(accumulatedWatch.current)
        );
      }
    };
  }, []);
      ...
      >
        <Video
          ...
          onPlaybackStatusUpdate={(status: any) => {
            ...
            durationRef.current = status.durationMillis || 1;
            progress.setValue(status.positionMillis || 0);

            if (!isActive) return;
            if (!status.isPlaying) return;

            const delta =
              status.positionMillis - lastPosition.current;

            // Ignore jumps (seek / replay)
            if (delta > 0 && delta < 2000) {
              accumulatedWatch.current += delta / 1000;
            }

            lastPosition.current = status.positionMillis;

            // Send reward every 10 seconds of real playback
            if (
              accumulatedWatch.current -
                lastSentReward.current >=
              10
            ) {
              lastSentReward.current =
                accumulatedWatch.current;
